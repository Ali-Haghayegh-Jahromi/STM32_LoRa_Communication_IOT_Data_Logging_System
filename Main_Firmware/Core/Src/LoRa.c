...
\* ----------------------------------------------------------------------------- */
uint8_t LoRa_transmit(LoRa* _LoRa, void* data, uint8_t length, uint16_t timeout){
	uint8_t read;
	...
	LoRa_BurstWrite(_LoRa, RegFiFo,(uint8_t*)data, length);
	LoRa_gotoMode(_LoRa, TRANSMIT_MODE);
	uint32_t loratimeot=HAL_GetTick();
	while(1){
		...
		{
			if((HAL_GetTick()-loratimeot)	>=	timeout)
			{
				LoRa_gotoMode(_LoRa, mode);
				return 0;
			}
